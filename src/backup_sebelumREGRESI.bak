...
void addLogEntry() {
    ...
    sprintf(logBuffer[logHead].timestamp, "%04d/%02d/%02d %02d:%02d:%02d", now.year(), now.month(), now.day(), now.hour(), now.minute(), now.second());
...
float f_Redup(float x)
{
    ...
    if (x > 5000 && x < 25000) return (25000.0 - x) / (25000.0 - 5000.0);
    return 0.0;
}
...
{
    if (x <= 5000 || x >= 65000) return 0.0;
    if (x > 5000 && x <= 35000) return (x - 5000.0) / (35000.0 - 5000.0);
    if (x > 35000 && x < 65000) return (65000.0 - x) / (65000.0 - 35000.0);
    return 0.0;
}
...
void handleRoot() {
    serveSpiffs("/index.html", "text/html");
}

void handleCSS() {
    serveSpiffs("/style.css", "text/css");
}
...
//handle untuk mengubah mode system biar tidak tabrakan sama handle relay dan fuzzy
void handleOverride() {
        ...
        if (relay == "main") {
            ...
            if (idx >= 0 && idx < 5) {
                if (!manualRelayState[idx]) {
                    ...
                    int activeCount = 0;
                    for (int i = 0; i < 5; i++) {
                        if (manualRelayState[i]) activeCount++;
                    }
                    if (activeCount >= 3) {
                        server.send(400, "text/plain", "MAKSIMAL 3 ZONA AKTIF | GAKUAT AKI NYA");
                        return;
                    }
...
void handleConfigPost() {
    if (server.hasArg("plain")) {
        ...
        if (!error) {
            if (doc["sensorInterval"].is<uint32_t>()) cfgSensorInterval = doc["sensorInterval"];
            if (doc["manualMode"].is<int>()) cfgManualMode = doc["manualMode"];
            if (doc["manualDur"].is<uint32_t>()) cfgManualDur = doc["manualDur"];
            server.send(200, "text/plain", "OK");
            return;
        }
    }
    ...
    while (1)
    {
        if (xSemaphoreTake(zonaMutex, pdMS_TO_TICKS(500)) == pdTRUE) {
            for(int i=0; i<5; i++) {
                ...
                float sVal = map(adc, 2662, 1269, 0, 100);
                zona[i].soil = constrain(sVal, 0.0f, 100.0f);
            }
            xSemaphoreGive(zonaMutex);
        }
...
void FuzzyTask(void *pv)
{
    while (1)
    {
        ...
        if (xSemaphoreTake(zonaMutex, pdMS_TO_TICKS(500)) == pdTRUE) {
            for (int i=0; i<5; i++) {
                float durasi = prosesFuzzyMamdani(zona[i].soil, temperature, lightLevel);
                zona[i].duration = durasi;
                if (durasi > 0) {
                    if (durasi <= 20) zona[i].mode = 1;
                    else if (durasi <= 40) zona[i].mode = 2;
                    else zona[i].mode = 3;
...
void ManualControlTask(void *pv)
{
    while (1)
    {
        if (systemMode == 1 && cfgManualMode == 1) {
            ...
            for (int i=0; i<5; i++) {
                if (manualRelayState[i] && manualTimerOff[i] > 0 && now >= manualTimerOff[i]) {
                    manualRelayState[i] = false;
                    digitalWrite(relayPin[i], RELAY_OFF);
                    manualTimerOff[i] = 0;
...
void setup()
{
    ...
    for (int i = 0; i < 5; i++) {
        xTaskCreate(IrrigationTask, "Irrigation", 2048, (void *)(uintptr_t)i, 3, &irrigationHandle[i]);
    }

    // Stack untuk WebTask sedikit lebih besar agar aman menangani HTTP request dan JSON
    xTaskCreate(WebTask, "Web", 8192, NULL, 2, &webHandle);
    
    // Task khusus untuk mode manual (menggantikan void loop)
    xTaskCreate(ManualControlTask, "ManualControl", 2048, NULL, 1, &manualControlHandle);
